...
for (const [book, records] of Object.entries(byBook)) fs.writeFileSync(path.join(out,'study',`${book}.json`), stableJson(records));
...
for (const [letter, records] of Object.entries(byLetter)) fs.writeFileSync(path.join(out,'dictionary',`${letter}.json`), stableJson(records));
...
const bookLines = CANONICAL_BOOKS.map((book) => `  ${JSON.stringify(book)}: () => require('../../assets/tyndale/study/${book}.json') as StudyResource[],`).join('\n');
const letters = Object.keys(byLetter).sort();
const letterLines = letters.map((letter) => `  ${JSON.stringify(letter)}: () => require('../../assets/tyndale/dictionary/${letter}.json') as DictionaryArticle[],`).join('\n');
