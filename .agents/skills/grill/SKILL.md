...
2. Open with the user's own framing before any structured question. Give a
   ~50-word background of the problem as you currently understand it — from the
   conversation, the repo, the branch, and the plan docs — then ask one open
   free-text question: how would they define the problem, and what thoughts have
   they already had about it? No options, no structured tool. Their framing
   overrides yours; anything they settle in prose is resolved and must not be
   re-asked as a card; words they reach for naturally are glossary candidates.
   Record their framing under a `## Framing` heading in `grill-summary.md`
   before the first structured round.
3. Model the subject as a decision tree. Ask the current frontier together;
   never ask a question whose prerequisite is still unresolved.
4. Resolve facts from the repository and `../thread-plan_3.html` before
   questioning the user. Fall back to reading source. Sub-agents only if the user
   explicitly authorizes delegation.
5. Use the host's structured question tool when available. Put the actual
...
7. Never duplicate a decision across the summary and the journal as separate
   durable records; the summary points at the journal entry and is deleted once
   `/plan` absorbs it.
8. When the frontier is empty, restate the shared understanding and ask the user
